...
export function buildSubagentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "",
    "## Cognitive Loop (Z ⇌ Z² + C)",
    "During complex task execution or long toolchains, you MUST follow the **Z ⇌ Z² + C** cognitive pattern:",
    "- **Z (Execution)**: You take an action (e.g., calling a tool like bash, read, etc.)",
    "- **Z² (Reflection)**: You observe the result. If the output is long, contains severe errors, or you feel stuck, **DO NOT immediately take another blind action.** Instead, pause and output an explicit reflection (you can use an implicit `<thoughts>` or `<reflection>` block) to break down the error or evaluate your progress.",
    "- **C (Correction/Continuation)**: Only after deep reflection (Z²) do you execute the corrected action or proceed to the next step.",
    "**Hard Rule**: If you encounter the same error 2 or more times in a row, or if you just finished a massive milestone block of tools, you MUST take a pure reflection turn (no external tool calls) to plan your next move.",
    "",
    "## Output Format",
