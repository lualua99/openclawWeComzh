...
## 🤖 深度解析：多智能体协作与拟人化操作演示 (Deep Dive)

OpenClaw 并非一个简单的对话窗口，而是一个具备**自主决策、长期记忆与多智能体协作**能力的数字员工。

### 1. 💬 Chat: 拟人化自然交流与上下文记忆 (Contextual Memory)
...
### 2. 🧠 私有知识库驱动 (Knowledge Base)

无缝接入本地文档与代码库。只需赋予 AI 读取权限，它就能像资深工程师一样自动检索当前项目中的最佳实践、说明手册和 API 规范，并基于这些私有知识为您解答问题、编写代码。

- **挂载与检索**：你可以要求助手读取特定目录（例如 `docs/` 或 `src/`）。智能体会自主调用工具（如 `list_dir` 和 `grep_search`）来精准定位信息。
- **防遗忘机制**：核心文档可作为知识组件（Knowledge Component）长期存在于智能体记忆中，面对复杂问题时甚至能在跨回话（Cross-Session）之间保持连贯性。

### 3. 🦞 Sandbox: 任务自动编排与多智能体协作 (Task Auto-Execution)

在左侧沙盘控制台中，你可以清晰地观察到 AI 是如何将一个复杂任务拆解为多步计划的。
在传统的 Chat 模式中，对话往往是一问一答。但在 Sandbox 模式中，AI 将转变为**自主工作台**模式：

1. **任务分解 (Task Breakdown)**：告知系统你要实现的目标（例如“帮我编写一个新功能并测试”），AI 会自动编写一份清晰的规划清单 (`task.md`)。
2. **打卡执行 (Self-Checking Execution)**：在执行每一步环境操作（终端命令、创建文件、查阅浏览器）后，AI 都会自动在任务面板上打勾标记，并调整下一步的**当前状态 (Task Status)**。
3. **人工接入防线 (Human-in-the-loop)**：当遇到高危终端指令（如 `rm -rf`或重启服务）时，沙盒将挂起操作并推送到控制台，等待你的人工审核 (Approve/Reject)。
   ![Sandbox Demo](docs/assets/sandbox_page_demo_1772675227672.png)

### 4. 🌐 Multi-Agent: 多智能体调度与协作流

对于复杂工作流，主智能体（`agent:main`）可以通过网关调度出不同专业领域的子智能体（如安全审计员、前端切图仔）来进行群体智能编排。

- **无干扰发包任务**：你可以直接对主 Agent 说：“**请启动一个具有文件写入权限的子智能体帮我重构 UI 层，由于我目前正在测试后端，请利用后台静默完成它**”
- 此时子智能体将脱离主线对话框，在后台建立一个平行的思考链路，你可以通过 **Overview** 页面实时查看所有活跃 Agent 的心跳和 Token 开销。

**系统级总览 (Gateway & Agents)**
控制台为您提供实时的网关连接、活动通道和实例调度情况。随时掌控这台“赛博龙虾中心”的算力运转情况。
