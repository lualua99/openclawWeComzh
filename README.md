...
- 🤖 **Multi-Agent 多智能体协同作战与记忆穿透**：引入基于网关的子智能体分发调度大盘。除了可以下发并行任务外，**主代理还可以向子代理传递结构化上下文，子任务完成后自动层层汇聚修改后的状态回传给父代理**，彻底解决多层智能体之间的信息割裂，实现跨任务的记忆闭环。
- 🧠 **本地私有知识库引擎挂载**：在多轮跨会话之间建立长线上下文记忆。直接让 AI 挂载本地项目文档和代码规范库，在遇到技术卡点时自主翻阅资料寻找突破。
- 💻 **沉浸式中文 UI 交互生态**：不仅整合了即开即用的终端向导 (`openclaw onboard`)，在大盘页面中还彻底重构了工具展示层。现在，AI 执行浏览器的截图、快照提取等操作时，**将直接在聊天框中呈现内联截图缩略图和卡片摘要**，过程透明可见。
- 🌍 **浏览器自动化持久化升级**：告别每次跑 AI 任务都要重新扫码登录的痛点。Agent 首次登录系统的浏览器身份（Cookie、localStorage 等）将与当前 Session 自动绑定保存，随叫随用。此外，深度强化了 `/batch` 操作的机制，极大减少了 AI 解析中间页面的无效 Token 消耗。
...
| **浏览器工具栏** | 仅显示一串返回代码  | 💬 **聊天框内联即时渲染出网页截图与组件快照摘要** |
| **浏览器自动化** | 每次执行前扫码登录  | 🌍 **自动关联绑定 Agent Session，跨任务免登**     |
