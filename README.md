...
| :------------- | :------------------ | :--------------------------------------- |
| **对话模式**   | 被动问答，一问一答  | 自主工作台，任务自驱执行                 |
| **任务追踪**   | 靠用户自己记录进度  | AI 自动生成 `task.md` 并实时打卡         |
| **高危操作**   | 直接执行，无拦截    | `Human-in-the-loop` 弹窗强制人工审核     |
| **智能体数量** | 单 Agent 串行       | 多 Agent 并行，网关统一调度              |
| **知识来源**   | 仅限对话上下文      | 支持挂载本地私有文档库、跨会话记忆       |
| **UI 语言**    | 英文界面，部分中文  | 全链路中文化，极致贴合国人习惯           |
...
│       ✅ Step 2: 分析依赖关系                               │
│       🔄 Step 3: 修改代码（进行中）                         │
│       ⚠️ Step 4: 执行 git push【等待人工审核】              │
└─────────────────────────────────────────────────────────────┘
```

#### 🛡️ Human-in-the-loop 高危拦截机制
...
AI 会自动打勾执行 1-3，但到第 4 步（`git push`）时会**暂停并推送弹窗让你审核**，保障操作安全。
...
```
    你（用户）
       │
       ▼
 [ agent:main ] ← 主脑（Claude Opus 4.6）负责决策与规划
       │
       ├──── 派发 ────► [ agent:kimi ] ← 负责日常沟通与中文文档
       │
       └──── 派发 ────► [ Subagent-1 ] ← 负责后台代码执行
                              │
                              └──► [ Subagent-2 ] ← 负责测试验证
```
...
```
我正在测试后端逻辑，请你同时启动一个子智能体帮我在后台重构
ui/src/ 目录的组件文件，完成后通知我，不要打断我现在的工作。
```
...
```
主脑 Opus：http://127.0.0.1:18789/chat?session=agent%3Amain%3Amain
